...
def create_executive_compensation_report():
    ...
    for company in data['companies']:
        if company['success'] and company['executives']:
            # Company summary
            total_exec_comp = sum(exec['total_compensation'] for exec in company['executives'])
            ...
            summary_data.append({
                ...
                'Highest_Paid_Executive': max(company['executives'], key=lambda x: x['total_compensation'])['name'],
                'Highest_Compensation': max(exec['total_compensation'] for exec in company['executives'])
            })
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        ...
        stats_data = {
            ...
            'Value': [
                ...
                len(executive_df),
                f"{len(executive_df) / data['successful_extractions']:.1f}" if data['successful_extractions'] > 0 else 0,
                f"${executive_df['Total_Compensation'].max():,.0f}" if not executive_df.empty else "$0",
                f"${executive_df['Total_Compensation'].mean():,.0f}" if not executive_df.empty else "$0",
                f"${executive_df['Total_Compensation'].sum():,.0f}" if not executive_df.empty else "$0"
            ]
        }
