...
class WeatherData(BaseModel):
    ...
    def validate_temperature(cls, v: float) -> float:
        ...
        if v < -50.0 or v > 50.0:
            raise ValueError(
                f"Temperature {v}°C is outside valid range [-50, 50]"
            )
        return v
    ...
    def validate_humidity(cls, v: int) -> int:
        ...
        if v < 0 or v > 100:
            raise ValueError(
                f"Humidity {v}% is outside valid range [0, 100]"
            )
        return v
...
class WeatherDataExtractor:
    ...
    def extract(self, raw_data: dict[str, Any]) -> WeatherData:
        ...
        if not isinstance(humidity, (int, float)):
            raise InvalidTypeError(
                f"Expected humidity to be numeric, "
                f"got {type(humidity).__name__}"
            )
        
        # Transform temperature from Kelvin to Celsius
        temperature_celsius = self._convert_kelvin_to_celsius(
            temperature_kelvin
        )
