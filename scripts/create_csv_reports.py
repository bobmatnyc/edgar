...
def create_csv_reports():
                ...
                executive_data.append(
                    {
                        ...
                        "Salary_Percent": (
                            exec["salary"] / exec["total_compensation"] * 100
                        )
                        if exec["total_compensation"] > 0
                        else 0,
                        "Bonus_Percent": (
                            exec["bonus"] / exec["total_compensation"] * 100
                        )
                        if exec["total_compensation"] > 0
                        else 0,
                        "Stock_Percent": (
                            exec["stock_awards"] / exec["total_compensation"] * 100
                        )
                        if exec["total_compensation"] > 0
                        else 0,
                        "Options_Percent": (
                            exec["option_awards"] / exec["total_compensation"] * 100
                        )
                        if exec["total_compensation"] > 0
                        else 0,
                    }
            ...
            company_summary.append(
                {
                    ...
                    "CEO_Compensation": next(
                        (
                            exec["total_compensation"]
                            for exec in company["executives"]
                            if "chief executive" in exec["title"].lower()
                            or "ceo" in exec["title"].lower()
                        ),
                        0,
                    ),
                }
    ...
    if not executive_df.empty:
        print("\n📈 Quick Statistics:")
        print(
            f"   💰 Highest Paid Executive: {executive_df.iloc[0]['Executive_Name']} at {executive_df.iloc[0]['Company_Name']}"
        )
        print(
            f"   💵 Highest Compensation: ${executive_df.iloc[0]['Total_Compensation']:,.0f}"
        )
        print(
            f"   📊 Average Compensation: ${executive_df['Total_Compensation'].mean():,.0f}"
        )
        print(
            f"   🎯 Median Compensation: ${executive_df['Total_Compensation'].median():,.0f}"
        )
