...
async def demo_file_operations():
    ...
    session = InteractiveExtractionSession(project_path=temp_dir, test_mode=True)
...
"""
    ...
    result1 = await session.scripting_engine.execute_script(
        script_code=script1, context={"base_dir": str(temp_dir)}, safety_checks=True
    )
...
"""
    ...
    result2 = await session.scripting_engine.execute_script(
        script_code=script2, context={}, safety_checks=True
    )
...
"""
    ...
    result3 = await session.scripting_engine.execute_script(
        script_code=script3, context={}, safety_checks=True
    )
