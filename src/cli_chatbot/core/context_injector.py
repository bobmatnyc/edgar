...
from .interfaces import ContextInfo, ContextProvider
...
class DynamicContextInjector(ContextProvider):
    ...
    def __init__(
        ...
        self.controller_root = (
            Path(controller_root) if controller_root else Path(__file__).parent.parent
        )
        ...
        logger.info(
            "Dynamic Context Injector initialized",
            app_root=str(self.application_root),
            controller_root=str(self.controller_root),
        )

    async def extract_context(
        self, query: str, application_root: str = None, max_contexts: int = 10
    ) -> List[ContextInfo]:
        ...
        doc_contexts = await self._extract_documentation_contexts(
            query, max_contexts // 2
        )
    ...
    async def get_controller_self_awareness(self) -> ContextInfo:
        ...
        for py_file in self.controller_root.rglob("*.py"):
            ...
            try:
                ...
                logger.warning(
                    "Failed to parse controller file", file=str(py_file), error=str(e)
                )
...
- Safety-first script execution with validation""",
    ...
    async def _extract_code_contexts(
        ...
        for py_file in self.application_root.rglob("*.py"):
            ...
            try:
                    ...
                    if isinstance(node, (ast.FunctionDef, ast.ClassDef)):
                        ...
                        if relevance > 0.1:  # Minimum relevance threshold
                            ...
                            contexts.append(
                                ContextInfo(
                                    source=str(
                                        py_file.relative_to(self.application_root)
                                    ),
                                    content_type=(
                                        "function"
                ...
                logger.debug(
                    "Failed to parse Python file", file=str(py_file), error=str(e)
                )

        return contexts[:max_contexts]

    async def _extract_documentation_contexts(
        self, query: str, max_contexts: int
    ) -> List[ContextInfo]:
                    ...
                    try:
                        ...
                        if relevance > 0.1:
                            contexts.append(
                                ContextInfo(
                                    source=str(
                                        doc_file.relative_to(self.application_root)
                                    ),
