...
class TraditionalCLI:
    ...
    async def analyze_codebase(
        ...
        analysis = {"total_contexts": len(contexts), "contexts": []}
    ...
    async def execute_script(
        self, script_code: str, context: Dict[str, Any] = None
    ) -> Dict[str, Any]:
    ...
    async def get_application_info(self) -> Dict[str, Any]:
        ...
        if self.application_root.exists():
            ...
            info["python_files"] = [
                str(f.relative_to(self.application_root)) for f in python_files[:20]
            ]
            ...
            directories = [
                d
                for d in self.application_root.iterdir()
            ...
            info["config_files"] = [
                str(f.relative_to(self.application_root)) for f in config_files[:10]
            ]
...
def create_fallback_cli(application_root: str) -> click.Group:
        ...
        async def run_analysis():
            ...
            if output_format == "json":
                ...
                for ctx_info in analysis["contexts"]:
                    click.echo(
                        f"• {ctx_info['source']} ({ctx_info['type']}) - {ctx_info['relevance']:.2f}"
                    )
            else:  # table format
                click.echo(
                ...
                for ctx_info in analysis["contexts"]:
                    ...
                    click.echo(
                        f"   Type: {ctx_info['type']} | Relevance: {ctx_info['relevance']:.2f}"
                    )
    ...
    @cli.command()
    @click.option(
        "--file", "-f", type=click.Path(exists=True), help="Execute script from file"
    )
