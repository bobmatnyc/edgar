...
from edgar_analyzer.services.parallel_processing_service import (
...
class Container(containers.DeclarativeContainer):
    ...
    edgar_api_service = providers.Singleton(
        EdgarApiService, config=config, cache_service=cache_service
    )
    ...
    report_service = providers.Singleton(
        ReportService, data_extraction_service=data_extraction_service, config=config
    )
