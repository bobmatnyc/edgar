...
class CompensationYear(BaseModel):
    ...
    year: int = Field(
        ...,
        description="Fiscal year", ge=2000, le=2100
    )
    
    salary: int = Field(
        default=0,
        description="Base salary ($)", ge=0
    )
    
    bonus: int = Field(
        default=0,
        description="Annual bonus ($)", ge=0
    )
    
    stock_awards: int = Field(
        default=0,
        description="Stock awards ($)", ge=0
    )
    
    total: int = Field(
        ...,
        description="Total compensation ($)", ge=0
    )
...
class ExecutiveCompensation(BaseModel):
    ...
    compensation_by_year: List[CompensationYear] = Field(
        ...,
        description="Compensation data for each fiscal year", min_length=1, max_length=3
    )
...
class SCTData(BaseModel):
    ...
    ticker: str = Field(
        ...,
        description="Stock ticker symbol", pattern=r"^[A-Z]{1,5}$"
    )
    ...
    executives: List[ExecutiveCompensation] = Field(
        ...,
        description="Named Executive Officers (typically 5)", min_length=1, max_length=10
    )
