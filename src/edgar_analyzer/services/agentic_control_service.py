...
class AgenticControlService:
    ...
    def __init__(self, llm_service: LLMService):
        ...
        logger.info(
            "Agentic Control Service initialized",
    ...
    async def evaluate_and_improve(
        ...
        """
        logger.info(
            f"Starting evaluation cycle {iteration+1}/{max_iterations}",
        ...
        checkpoint_id = await self._create_git_checkpoint(
            f"pre_improvement_{iteration}"
        )
        ...
        try:
            ...
            # Rollback on error
            logger.error(
                "Error during improvement, rolling back",
                error=str(e),
    ...
    async def _supervisor_evaluate(
        self, results: List[Dict[str, Any]], company: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            content = await self.llm_service._make_llm_request(
                [
                    {
                        "role": "system",
                        "content": f"You are the Supervisor AI of an agentic extraction system. Use {self.supervisor_model} capabilities for strategic evaluation.",
                    },
            ...
            return {"quality_score": 0.0, "needs_improvement": True, "error": str(e)}
