...
class AutoResumeService:
    ...
    def analyze_resume_options(
        self, target_year: int, company_count: int, max_age_hours: int = 24
    ) -> Dict[str, any]:
            ...
            if cp.get("target_year") == target_year:
                try:
                    ...
                    if age_hours <= max_age_hours:
                        ...
                        cp["company_count_diff"] = abs(
                            cp.get("total_companies", 0) - company_count
                        )
        ...
        incomplete = [
            cp for cp in relevant_checkpoints if cp.get("progress", 100) < 100
        ]
        ...
        if incomplete:
            ...
            incomplete.sort(
                key=lambda cp: (-cp.get("progress", 0), cp["company_count_diff"])
            )
            ...
            recent_complete = [
                cp for cp in complete if cp["age_hours"] < 6
            ]  # Last 6 hours

            if recent_complete:
                candidate = recent_complete[0]
                if (
                    candidate["company_count_diff"] <= company_count * 0.1
                ):  # 10% tolerance
                    best_candidate = candidate
