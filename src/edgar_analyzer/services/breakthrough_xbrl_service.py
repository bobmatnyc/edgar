...
class BreakthroughXBRLService:
    ...
    async def extract_executive_compensation(self, symbol: str, company_name: str) -> Dict:
        ...
        try:
            ...
            if not proxy_filings:
                return self._create_error_result(symbol, company_name, "no_proxy_filings")
            ...
            executives = await self._extract_from_pvp_disclosure(xbrl, symbol, company_name)
            
            if executives:
                logger.info(f"✅ BREAKTHROUGH: Extracted {len(executives)} executives from XBRL")
                return {
                    'success': True,
                ...
                return self._create_error_result(symbol, company_name, "no_executives_extracted")
                
        except Exception as e:
            logger.error(f"Error in breakthrough extraction for {symbol}: {e}")
            return self._create_error_result(symbol, company_name, "extraction_error", str(e))
    
    async def _extract_from_pvp_disclosure(self, xbrl, symbol: str, company_name: str) -> List[Dict]:
    ...
    def _extract_peo_compensation(self, facts_df: pd.DataFrame, symbol: str) -> Optional[Dict]:
        ...
        try:
            ...
            latest_total = peo_total_facts.sort_values('numeric_value', ascending=False).iloc[0]
            ...
            if len(peo_name_facts) > 0:
                ...
                if isinstance(name_value, str):
                    ...
                    if "Cook" in name_value:
                        ...
                        peo_name = name_value.replace("Mr. ", "").replace("Ms. ", "").strip()

            # Get actually paid compensation if available
            actually_paid = 0
            if len(peo_paid_facts) > 0:
                latest_paid = peo_paid_facts.sort_values('numeric_value', ascending=False).iloc[0]
    ...
    def _extract_neo_compensation(self, facts_df: pd.DataFrame, symbol: str) -> List[Dict]:
        ...
        try:
            ...
            latest_total = neo_total_facts.sort_values('numeric_value', ascending=False).iloc[0]
            avg_total_compensation = int(latest_total['numeric_value'])

            # Get actually paid average if available
            avg_actually_paid = 0
            if len(neo_paid_facts) > 0:
                latest_paid = neo_paid_facts.sort_values('numeric_value', ascending=False).iloc[0]
            ...
            for i, name in enumerate(neo_names):
                ...
                logger.info(f"✅ Extracted NEO: {name} - ${avg_total_compensation:,} (average)")
    ...
    def _extract_neo_names_from_dimensions(self, facts_df: pd.DataFrame, symbol: str) -> List[str]:
        ...
        try:
            ...
            logger.info(f"Extracted {len(names)} NEO names from dimensions for {symbol}")

        except Exception as e:
            logger.error(f"Error extracting NEO names from dimensions for {symbol}: {e}")

        return names

    def _extract_neo_names_from_text(self, facts_df: pd.DataFrame, symbol: str) -> List[str]:
        ...
        try:
            ...
            text_block_mask = facts_df['concept'].str.contains('NamedExecutiveOfficersFnTextBlock', na=False)
    ...
    def _create_error_result(self, symbol: str, company_name: str, reason: str, error: str = None) -> Dict:
