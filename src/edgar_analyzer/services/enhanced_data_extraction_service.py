...
from edgar_analyzer.services.interfaces import ICompanyService, IEdgarApiService
...
class EnhancedDataExtractionService:
    ...
    def __init__(
        ...
        if edgar_api_service and company_service:
            self.edgar_service = DataExtractionService(
                edgar_api_service, company_service
            )

        logger.info(
            f"Enhanced data extraction service initialized: FMP enabled, EDGAR fallback: {self.edgar_service is not None}"
        )

    async def extract_executive_compensation(
        self, company_name: str, rank: int, cik: Optional[str] = None, year: int = 2023
    ) -> Optional[Dict]:
        """Extract executive compensation using FMP API with EDGAR fallback"""

        logger.info(
            f"Starting enhanced compensation extraction for {company_name} (rank {rank}, year {year})"
        )
        ...
        if fmp_result and self._validate_fmp_data(fmp_result):
            logger.info(
                f"Successfully extracted from FMP API for {company_name}: {len(fmp_result['executives'])} executives"
            )
            return fmp_result
        ...
        if self.edgar_service and cik:
            logger.info(
                f"Falling back to EDGAR extraction for {company_name} (CIK: {cik})"
            )
            edgar_result = await self._extract_from_edgar(cik, company_name, rank)

            if edgar_result and edgar_result.get("success"):
                logger.info(
                    f"Successfully extracted from EDGAR for {company_name}: {len(edgar_result.get('executives', []))} executives"
                )
                return edgar_result
    ...
    async def _extract_from_fmp(
        self, company_name: str, rank: int, year: int
    ) -> Optional[Dict]:
    ...
    async def _extract_from_edgar(
        self, cik: str, company_name: str, rank: int
    ) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(
                f"EDGAR extraction failed for {company_name} (CIK: {cik}): {str(e)}"
            )
            return None
    ...
    def _validate_fmp_data(self, data: Dict) -> bool:
        ...
        for exec_data in executives:
            ...
            if total_comp < 100_000 or total_comp > 1_000_000_000:
                logger.warning(
                    f"Compensation out of reasonable range for {name}: {total_comp}"
                )
                return False
    ...
    async def batch_extract_compensation(
        self, companies: List[Dict], year: int = 2023, max_concurrent: int = 5
    ) -> List[Dict]:
        """Extract compensation for multiple companies concurrently"""

        logger.info(
            f"Starting batch compensation extraction for {len(companies)} companies (year {year})"
        )
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                logger.error(
                    f"Extraction failed with exception for {companies[i]['name']}: {str(result)}"
                )
        ...
        logger.info(
            f"Batch extraction completed: {successful}/{len(companies)} successful, {failed} failed"
        )
