...
class EnhancedTableParser:
    ...
    def _score_headers(self, headers: List[str]) -> float:
        ...
        for header in headers:
            ...
            # High value headers
            if any(comp_header in header_lower for comp_header in 
                   ['name', 'total', 'salary', 'bonus', 'stock', 'option']):
                score += 0.2
            
            # Medium value headers
            elif any(comp_header in header_lower for comp_header in 
                     ['compensation', 'awards', 'incentive', 'year']):
                score += 0.1
            
            # Executive title indicators
            elif any(re.search(pattern, header_lower) for pattern in self.title_patterns):
                score += 0.15
    ...
    def _analyze_table_structure(self, table: Tag) -> float:
        ...
        for row in rows[1:]:  # Skip header row
            ...
            if cells:
                ...
                elif any(re.search(pattern, first_cell.lower()) for pattern in self.title_patterns):
                    name_indicators += 1
