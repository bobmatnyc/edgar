...
class FMPApiService:
    ...
    async def get_executive_compensation(
        ...
        try:
            ...
            params = {"symbol": symbol, "year": year, "apikey": self.api_key}
            ...
            async with session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    if data and isinstance(data, list) and len(data) > 0:
                        logger.info(
                            f"Successfully retrieved compensation data for {symbol}: {len(data)} executives"
                        )
                        return {
                            "symbol": symbol,
                            "year": year,
    ...
    def format_compensation_data(
        self, fmp_data: Dict, company_name: str, rank: int
    ) -> Dict:
        ...
        for exec_data in fmp_data.get("executives", []):
            ...
            executive = {
                ...
                "total_compensation": self._safe_int(
                    exec_data.get("totalCompensation", 0)
                ),
                ...
                "other_compensation": self._safe_int(
                    exec_data.get("otherCompensation", 0)
                ),
            }
