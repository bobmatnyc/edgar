...
class ModelConfigService:
    ...
    def __init__(self):
        ...
        logger.info(
            "Model configuration service initialized",
    ...
    def get_best_model_for_context_size(self, required_context: int) -> str:
        ...
        if not suitable_models:
            logger.warning(
                f"No model found for context size {required_context}, using default"
            )
        ...
        suitable_models.sort(
            key=lambda x: (
                cost_order.get(x[1].get("cost", "premium"), 3),
                -x[1].get("context", 0),
            )
        )
    ...
    def recommend_model_for_use_case(
        ...
        if any(
            ...
            for keyword in ["code", "programming", "engineer", "development"]
        ):
            task = "coding"
        elif any(
            keyword in use_case_lower
            for keyword in ["qa", "quality", "test", "validation"]
        ):
            task = "qa_supervision"
        elif any(
            keyword in use_case_lower
            for keyword in ["search", "web", "current", "real-time"]
        ):
            task = "web_search"
        elif any(
            keyword in use_case_lower for keyword in ["analysis", "analyze", "data"]
        ):
            task = "analysis"
        else:
            task = "conversation"
        ...
        if context_size:
            ...
            if context_model != model:
                logger.info(
                    f"Switching from {model} to {context_model} for context size {context_size}"
                )
                model = context_model
        ...
        if web_search_required:
            ...
            if not model_info.get("web_search", False):
                web_search_models = [
                    m
                    for m, info in self.model_info.items()
                    if info.get("web_search", False)
                ]
