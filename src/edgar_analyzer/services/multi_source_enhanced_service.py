...
class MultiSourceEnhancedService:
    ...
    def __init__(self, identity: str = "edgar.analyzer@example.com"):
        ...
        logger.info(
            f"Available sources: XBRL, SEC-API.io: {bool(self.sec_api_key)}, FMP: {bool(self.fmp_api_key)}, AI: {bool(self.openai_api_key)}"
        )
    ...
    async def extract_executive_compensation(
        ...
        try:
            ...
            xbrl_result = await self.xbrl_service.extract_executive_compensation(
                symbol, company_name
            )
        ...
        if self.sec_api_key:
            try:
                ...
                attempts.append(
                    ("sec_api_professional", {"success": False, "error": str(e)})
                )
        ...
        if self.openai_api_key:
            try:
                ...
                attempts.append(
                    ("ai_llm_extraction", {"success": False, "error": str(e)})
                )
    ...
    async def _extract_from_sec_api(self, symbol: str, company_name: str) -> Dict:
        ...
        try:
            ...
            params = {"token": self.sec_api_key, "ticker": symbol}
                    ...
                    if response.status == 200:
                        ...
                        for exec_data in data.get("executives", []):
                            executive = {
                                ...
                                "total_compensation": exec_data.get(
                                    "totalCompensation", 0
                                ),
                                ...
                                "other_compensation": exec_data.get(
                                    "otherCompensation", 0
                                ),
                        ...
                        return {
                            "success": False,
                            "error": f"SEC-API.io returned status {response.status}",
                        }
                ...
                async with session.get(url, params=params) as response:
                    if response.status == 200:
                        ...
                        for exec_data in data:
                            executive = {
                                "name": exec_data.get("nameAndTitle", "Unknown")
                                .split(",")[0]
                                .strip(),
                                "title": (
                                    exec_data.get("nameAndTitle", "Unknown")
                                    .split(",")[-1]
                                    .strip()
                                    if "," in exec_data.get("nameAndTitle", "")
                                    else "Unknown"
                                ),
                                "total_compensation": exec_data.get(
                                    "totalCompensation", 0
                        ...
                        return {
                            "success": False,
                            "error": f"FMP returned status {response.status}",
                        }
