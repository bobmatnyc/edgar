...
class ComprehensiveQAController:
    ...
    async def qa_executive_data(self, company_data: Dict) -> QAResult:
        ...
        for i, exec_data in enumerate(executives):
            cleaned_exec = await self._clean_executive_data(
                exec_data, company_name, corrections
            )
            if cleaned_exec:
                ...
                "qa_metadata": {
                    ...
                    "major_issues": len(
                        [
                            i
                            for i in issues
                            if "invalid" in i.lower() or "artificial" in i.lower()
                        ]
                    ),
                },
            },
    ...
    async def _validate_executive_names(
        ...
        for i, exec_data in enumerate(executives):
            ...
            if cleaned_name != name:
                ...
                corrections[f"executive_{i}_title_from_name"] = name.replace(
                    cleaned_name, ""
                ).strip()
            ...
            if self.web_search_enabled and self.llm_service:
                try:
                    is_valid_exec = await self._web_validate_executive(
                        company_name, cleaned_name
                    )
                    if not is_valid_exec:
                        issues.append(
                            f"Executive '{cleaned_name}' not found at {company_name}"
                        )
    ...
    def _validate_compensation_patterns(
        self, executives: List[Dict], company_rank: int
    ) -> Dict:
        ...
        for i, exec_data in enumerate(executives):
            ...
            if total_comp > 0:
                ...
                for artificial_ratio in self.artificial_ratios:
                    if self._ratios_match(ratios, artificial_ratio, tolerance=0.001):
                        ...
                        issues.append(
                            f"Executive {i+1}: Artificial compensation ratio detected"
                        )
            ...
            if total_comp_millions < comp_range["min"]:
                issues.append(
                    f"Executive {i+1}: Compensation unusually low (${total_comp_millions:.1f}M)"
                )
            elif total_comp_millions > comp_range["typical_max"]:
                issues.append(
                    f"Executive {i+1}: Compensation unusually high (${total_comp_millions:.1f}M)"
                )

        # Flag if too many artificial patterns
        if artificial_pattern_count >= len(executives) * 0.8:
            issues.append(
                "Majority of executives show artificial compensation patterns"
            )
    ...
    def _validate_titles(self, executives: List[Dict]) -> Dict:
        ...
        for i, exec_data in enumerate(executives):
            ...
            if not any(
                re.search(pattern, title) for pattern in self.valid_title_patterns
            ):
        ...
        ceo_count = sum(
            1 for title in titles if "ceo" in title or "chief executive" in title
        )
        if ceo_count > 1:
    ...
    async def _clean_executive_data(
        self, exec_data: Dict, company_name: str, corrections: Dict
    ) -> Optional[Dict]:
    ...
    def _is_invalid_name(self, name: str) -> bool:
        ...
        return any(
            re.search(pattern, name_lower) for pattern in self.invalid_name_patterns
        )
    ...
    def _ratios_match(
        self,
        ratios1: Tuple[float, float, float, float],
    ...
    async def _web_validate_executive(self, company_name: str, exec_name: str) -> bool:
        ...
        try:
            ...
            response = await self.llm_service.web_search_request(
                query, f"Validating if {exec_name} is an executive at {company_name}"
            )
            ...
            if response:
                ...
                return (
                    exec_name.lower() in response_lower
                    and company_name.lower() in response_lower
                )
    ...
    def _calculate_confidence_score(
        self, issues: List[str], executive_count: int
    ) -> float:
