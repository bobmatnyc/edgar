...
class EnhancedValidationService:
    ...
    async def validate_executive_data(
        self, company_name: str, company_rank: int, executives: List[Dict]
    ) -> ValidationResult:
        ...
        try:
            ...
            name_validation = await self._validate_executive_names(
                company_name, executives
            )
            ...
            comp_validation = self._validate_compensation_ranges(
                company_rank, executives
            )
    ...
    async def _validate_executive_names(
        ...
        try:
            ...
            search_response = await self.web_search_client(
                search_query, f"Finding executives for {company_name}"
            )
            ...
            if search_response:
                ...
                for exec_data in executives:
                    ...
                    if not name_match:
                        ...
                        exec_search = await self.web_search_client(
                            exec_search_query,
                            f"Validating {exec_name} at {company_name}",
                        )
                        ...
                        if exec_search:
                            ...
                            if not self._validate_executive_at_company_text(
                                exec_name, company_name, exec_search
                            ):
                                issues.append(
                                    f"Executive '{exec_name}' not found at {company_name}"
                                )
                                suggestions.append(
                                    f"Verify '{exec_name}' is current executive at {company_name}"
                                )
    ...
    def _validate_compensation_ranges(
        self, company_rank: int, executives: List[Dict]
    ) -> Dict:
        ...
        for exec_data in executives:
            total_comp = (
                exec_data.get("total_compensation", 0) / 1_000_000
            )  # Convert to millions
            ...
            if any(pattern in title for pattern in ["ceo", "chief executive"]):
                if total_comp < benchmark.min_ceo_comp:
                    issues.append(
                        f"CEO {name} compensation (${total_comp:.1f}M) below expected range (${benchmark.min_ceo_comp}M-${benchmark.max_ceo_comp}M)"
                    )
                elif total_comp > benchmark.max_ceo_comp:
                    suggestions.append(
                        f"CEO {name} compensation (${total_comp:.1f}M) above typical range - verify accuracy"
                    )
            ...
            # Check CFO compensation
                ...
                if total_comp < benchmark.min_cfo_comp:
                    issues.append(
                        f"CFO {name} compensation (${total_comp:.1f}M) below expected range"
                    )
                elif total_comp > benchmark.max_cfo_comp:
                    suggestions.append(
                        f"CFO {name} compensation (${total_comp:.1f}M) above typical range"
                    )
            ...
            # Check other executives
                ...
                if total_comp < benchmark.min_other_exec_comp:
                    issues.append(
                        f"Executive {name} compensation (${total_comp:.1f}M) unusually low"
                    )
                elif total_comp > benchmark.max_other_exec_comp:
                    suggestions.append(
                        f"Executive {name} compensation (${total_comp:.1f}M) unusually high"
                    )
    ...
    def _validate_executive_titles(self, executives: List[Dict]) -> Dict:
        ...
        for title, count in title_counts.items():
            if count > 1 and "ceo" in title:
                ...
                suggestions.append(
                    f"Multiple executives with same title: '{title}' ({count} times)"
                )
        ...
        for exec_data in executives:
            ...
            if not title or title.strip() == "":
                ...
                re.search(pattern, title) for pattern in self.exec_title_patterns
            ):
    ...
    def _detect_artificial_patterns(self, executives: List[Dict]) -> Dict:
        ...
        if len(ratios) > 1:
            ...
            identical_count = sum(
                1 for ratio in ratios if self._ratios_similar(first_ratio, ratio)
            )

            if identical_count >= len(ratios) * 0.8:  # 80% or more have same ratios
                issues.append(
                    "Artificial compensation pattern detected: identical ratios across executives"
                )
                suggestions.append(
                    "Verify compensation data - ratios appear artificially uniform"
                )
        ...
        if (
            round_number_count >= len(executives) * 0.6
        ):  # 60% or more are round millions
            suggestions.append(
                "Many executives have round million compensation - verify accuracy"
            )
    ...
    def _validate_executive_at_company_text(
        self, exec_name: str, company_name: str, search_text: str
    ) -> bool:
    ...
    def _ratios_similar(
        self,
        ratio1: Tuple[float, float, float, float],
