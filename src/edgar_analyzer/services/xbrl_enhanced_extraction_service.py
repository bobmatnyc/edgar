...
class XBRLEnhancedExtractionService:
    ...
    def __init__(self, identity: str = "edgar.analyzer@example.com"):
        ...
        logger.info(f"XBRL Enhanced Extraction Service initialized with identity: {identity}")
    ...
    async def extract_executive_compensation(self, symbol: str, company_name: str) -> Dict:
        ...
        try:
            ...
            logger.info(f"Found proxy filing {proxy_filing.accession_number} dated {proxy_filing.filing_date}")
            
            # Try XBRL extraction first
            xbrl_result = await self._extract_xbrl_compensation(proxy_filing, symbol, company_name)
            if xbrl_result['success']:
                logger.info(f"XBRL extraction successful for {symbol}")
                return xbrl_result
            
            # Fallback to enhanced text extraction
            text_result = await self._extract_text_compensation(proxy_filing, symbol, company_name)
    ...
    async def _extract_xbrl_compensation(self, filing: Filing, symbol: str, company_name: str) -> Dict:
        ...
        try:
            ...
            logger.info(f"XBRL instance found for {symbol} - extracting structured data")
    ...
    async def _extract_text_compensation(self, filing: Filing, symbol: str, company_name: str) -> Dict:
        ...
        try:
            ...
            logger.info(f"Retrieved text content for {symbol}: {len(text_content)} characters")
            
            # Enhanced text parsing for executive compensation
            executives = await self._parse_executive_compensation_text(text_content, symbol)
    ...
    async def _parse_executive_compensation_text(self, text: str, symbol: str) -> List[Dict]:
        ...
        summary_match = re.search(summary_table_pattern, text, re.IGNORECASE | re.DOTALL)
    ...
    def _parse_compensation_table(self, table_text: str, symbol: str) -> List[Dict]:
        ...
        for line in lines:
            ...
            if any(title in line.lower() for title in ['ceo', 'chief executive', 'president', 'cfo', 'chief financial']):
                ...
                if compensation_match:
                    ...
                    executive = {
                        ...
                        'total_compensation': int(total_comp) if total_comp.isdigit() else 0,
    ...
    def _parse_individual_executive_sections(self, text: str, symbol: str) -> List[Dict]:
