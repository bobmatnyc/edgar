...
class Fortune500Builder:
    ...
    async def build_fortune500_database(
        self, output_file: str = "data/companies/fortune_500_complete.json"
    ) -> List[Company]:
        ...
        for company_data in self.known_companies:
            try:
                ...
                logger.debug(
                    "Added company", name=company.name, rank=company.fortune_rank
                )
    ...
    async def _save_companies_to_file(
        self, companies: List[Company], output_file: str
    ) -> None:
        ...
        try:
            ...
            logger.info(
                "Companies saved to file", file=str(output_path), count=len(companies)
            )
