...
class ValidationResult(BaseModel):
    ...
    expected_value: Optional[Union[str, float, int]] = Field(
        None, description="Expected value"
    )
    actual_value: Optional[Union[str, float, int]] = Field(
        None, description="Actual value"
    )
    suggestion: Optional[str] = Field(
        None, description="Suggestion for fixing the issue"
    )
...
class DataValidator:
    ...
    def validate_tax_expense(self, tax_expense: TaxExpense) -> List[ValidationResult]:
        ...
        current_tax = (
            ...
            if tax_expense.current_tax_expense is not None
            else 0.0
        )
        deferred_tax = (
            float(tax_expense.deferred_tax_expense)
            if tax_expense.deferred_tax_expense is not None
            else 0.0
        )
        calculated_total = current_tax + deferred_tax

        if abs(calculated_total - total_tax) > max(
            abs(total_tax) * 0.01, 1000
        ):  # 1% or $1K tolerance
            results.append(
                ValidationResult(
                    is_valid=False,
    ...
    def validate_executive_compensation(
        self, compensation: ExecutiveCompensation
        ...
        components = [
            ...
            (
                float(compensation.stock_awards)
                if compensation.stock_awards is not None
                else 0.0
            ),
            (
                float(compensation.option_awards)
                if compensation.option_awards is not None
                else 0.0
            ),
            (
                float(compensation.other_compensation)
                if compensation.other_compensation is not None
                else 0.0
            ),
        ]
        calculated_total = sum(components)

        if abs(calculated_total - total_comp) > max(
            abs(total_comp) * 0.01, 1000
        ):  # 1% or $1K tolerance
            results.append(
                ValidationResult(
                    is_valid=False,
    ...
    def _validate_executive_title(self, title: str) -> ValidationResult:
        ...
        return ValidationResult(
            ...
            actual_value=title,
            suggestion=(
                "Verify title extraction if unusual" if not is_common_title else None
            ),
        )
