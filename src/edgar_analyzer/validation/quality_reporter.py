...
class QualityReporter:
    ...
    async def generate_quality_report(
        self, analyses: List[CompanyAnalysis], report_name: str = "data_quality_report"
    ) -> Dict[str, any]:
    ...
    async def _validate_company_analysis(self, analysis: CompanyAnalysis) -> List[Dict]:
        ...
        for compensation in analysis.executive_compensations:
            comp_validations = self.data_validator.validate_executive_compensation(
                compensation
            )
        ...
        sanity_validations = self.sanity_checker.perform_comprehensive_sanity_check(
            analysis
        )
        ...
        if analysis.tax_expenses and analysis.executive_compensations:
            ...
            latest_comps = [
                c
                for c in analysis.executive_compensations
    ...
    def _generate_summary_statistics(
        self, all_validations: List[Dict], company_scores: Dict[str, Dict]
    ) -> Dict[str, any]:
        ...
        if company_scores:
            overall_score = sum(
                score["score"] for score in company_scores.values()
            ) / len(company_scores)
        else:
            overall_score = 0.0
    ...
    async def _export_quality_reports(
        self, report_data: Dict, report_name: str
    ) -> None:
    ...
    async def _create_excel_quality_report(
        self, report_data: Dict, excel_path: Path
    ) -> None:
    ...
    def _create_company_scores_sheet(
        self, workbook: Workbook, report_data: Dict
    ) -> None:
        """Create company scores sheet."""
