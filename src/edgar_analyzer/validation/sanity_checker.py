...
class SanityChecker:
    ...
    def perform_comprehensive_sanity_check(
    ...
    def _check_data_consistency(
        ...
        actual_comp_years = len(
            set(comp.fiscal_year for comp in analysis.executive_compensations)
        )
    ...
    def _check_cross_year_consistency(
        self, analysis: CompanyAnalysis
    ) -> List[ValidationResult]:
    ...
    def _check_industry_benchmarks(
        self, analysis: CompanyAnalysis
    ) -> List[ValidationResult]:
    ...
    def _check_logical_relationships(
        self, analysis: CompanyAnalysis
    ) -> List[ValidationResult]:
    ...
    def _detect_outliers(self, analysis: CompanyAnalysis) -> List[ValidationResult]:
        ...
        if len(analysis.tax_expenses) >= 3:
            tax_values = [
                float(tax.total_tax_expense)
                for tax in analysis.tax_expenses
                if tax.total_tax_expense is not None
            ]
            if len(tax_values) >= 3:
