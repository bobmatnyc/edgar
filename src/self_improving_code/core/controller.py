...
class SelfImprovingController:
    ...
    def __init__(
        ...
        logger.info("Self-Improving Controller initialized",
    ...
    async def improve_implementation(
        ...
        logger.info("Starting self-improving process",
        ...
        for iteration in range(max_iterations):
            ...
            try:
                ...
                if changes.get('changes_made', False):
                    ...
                    logger.info("Code improvements applied",
                               files_modified=len(files_modified))
                else:
                    logger.warning("Engineer could not implement improvements")

            except Exception as e:
                logger.error("Error during improvement iteration",
                           iteration=iteration + 1, error=str(e))

                # Rollback on error
                if iteration_result.checkpoint_id:
                    self.safety_manager.rollback_to_checkpoint(iteration_result.checkpoint_id)
        ...
        return FinalResult(
            ...
            improvements_made=[f for iter_result in iterations for f in iter_result.files_modified],
