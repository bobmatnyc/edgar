...
Extracts sct data from SEC filings using Claude Sonnet 4.5 via OpenRouter.

Process:
1. Fetch filing HTML from SEC EDGAR
2. Extract sct section using pattern matching
...
Example:
    >>> service = SCTExtractor(openrouter_client)
...
class SCTExtractor:
    """
    Service for extracting sct data from SEC filings.
    ...
    async def extract(
        ...
        """
        Extract sct data from SEC filing.
        ...
        try:
            ...
            debug_dir = Path(__file__).parent.parent.parent / "output" / "sct_debug"
            debug_dir.mkdir(parents=True, exist_ok=True)
            debug_file = debug_dir / f"{cik}_sct_section.html"
            with open(debug_file, "w") as f:
                f.write(section_html)
            logger.debug(f"Saved sct HTML to {debug_file}")
    ...
    def _build_extraction_prompt(
        ...
        prompt = f"""# Task: Extract sct Data
...
## Your Task
Extract structured data from the sct section in the HTML below.
...
"""
    ...
    def _validate_extraction(self, data: SCTData) -> bool:
        """
