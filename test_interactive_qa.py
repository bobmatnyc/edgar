...
class InteractiveTester:
    ...
    async def test_command(self, name: str, command: str, args: str = "", expect_error: bool = False) -> Dict:
    ...
    async def test_natural_language(self) -> List[Dict]:
        ...
        for query, expected_cmd, expected_args in nl_queries:
            ...
            print(f"   {status} Parsed as: {cmd} {args} (expected: {expected_cmd} {expected_args})")
    ...
    async def test_error_handling(self) -> List[Dict]:
        ...
        error_cases = [
            ...
            ("Generate without analyze", "generate", "", False),  # Should work with previous state
        ]
    ...
    async def test_session_persistence(self) -> Dict:
        ...
        print(f"   {'✅' if restore_success else '❌'} Session restored with project config")
    ...
    def generate_report(
        ...
        persist_success = persistence_results["save_success"] and persistence_results["restore_success"]
        ...
        print(f"🎯 OVERALL: {total_pass}/{total_tests} tests passed ({total_pass/total_tests*100:.1f}%)")

        # Production readiness check
        is_ready = (
            basic_pass == basic_total and
            nl_pass >= nl_total * 0.8 and  # 80% NL accuracy acceptable
            error_pass == error_total and
            persist_success and
            performance_results['full_workflow'] < 10.0
        )
