...
def analyze_complete_fortune_100():
    ...
    try:
        ...
        for _, row in fortune_1_8.iterrows():
            ...
            print(f'    Quality: {quality} | Confidence: {confidence} | Source: {source}')
            print()
        ...
        print(f'\n📈 FORTUNE RANKING ANALYSIS:')
        fortune_1_50 = df_executives[df_executives['Company'].isin(
            df_findings[df_findings['Fortune Rank'] <= 50]['Company']
        )]
        fortune_51_100 = df_executives[df_executives['Company'].isin(
            df_findings[df_findings['Fortune Rank'] > 50]['Company']
        )]
        ...
        if len(fortune_1_50) > 0:
            print(f'     • Avg Executive Pay: ${fortune_1_50["Average Annual Pay"].mean():,.0f}')
            print(f'     • Total Compensation: ${fortune_1_50["Average Annual Pay"].sum()/1_000_000_000:.1f}B')
        
        print(f'   Fortune 51-100:')
        print(f'     • Companies: {len(df_findings[df_findings["Fortune Rank"] > 50])}')
        print(f'     • Executives: {len(fortune_51_100)}')
        if len(fortune_51_100) > 0:
            print(f'     • Avg Executive Pay: ${fortune_51_100["Average Annual Pay"].mean():,.0f}')
            print(f'     • Total Compensation: ${fortune_51_100["Average Annual Pay"].sum()/1_000_000_000:.1f}B')
        
        # Real vs other data comparison
        real_sec_execs = df_executives[df_executives['Data Source'] == 'real_sec_filings']
        ...
        if len(real_sec_execs) > 0:
            print(f'     • Avg Pay: ${real_sec_execs["Average Annual Pay"].mean():,.0f}')
            print(f'     • Total: ${real_sec_execs["Average Annual Pay"].sum()/1_000_000:.1f}M')
        ...
        if len(other_execs) > 0:
            ...
            print(f'     • Total: ${other_execs["Average Annual Pay"].sum()/1_000_000_000:.1f}B')
        ...
        print(f'   ✅ Tim Cook now #1 highest paid: ${df_executives.iloc[0]["Average Annual Pay"]:,.0f}')
