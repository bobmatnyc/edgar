...
def analyze_fixed_fortune_100():
    ...
    try:
        ...
        fortune_1_8_execs = df_executives[
            df_executives["Company"].isin(fortune_1_8_companies)
        ]
        fortune_9_100_execs = df_executives[
            ~df_executives["Company"].isin(fortune_1_8_companies)
        ]
        ...
        print(
            f"   Fortune 1-8: {len(fortune_1_8_execs)} executives, avg pay: ${f18_avg:,.0f}"
        )
        print(
            f"   Fortune 9-100: {len(fortune_9_100_execs)} executives, avg pay: ${f9100_avg:,.0f}"
        )
        ...
        for i, company_name in enumerate(all_companies, 1):
            if company_name in fortune_1_8_companies:
                status = "✅ INCLUDED"
                company_data = fortune_1_8[fortune_1_8["Company"] == company_name].iloc[
                    0
                ]
                ...
                print(
                    f"   {i:2d}. {company_name}: {status} ({quality}, {confidence}) - ${total_pay:,.0f}"
                )
            else:
