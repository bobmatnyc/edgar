...
def analyze_fortune_100():
    ...
    try:
        ...
        ceo_data = df_breakdown[
            df_breakdown["Title"].str.contains("Chief Executive", na=False)
        ]
        ...
        for i, (_, exec_row) in enumerate(top_10.iterrows(), 1):
            print(
                f'   {i:2d}. {exec_row["Executive Name"]} ({exec_row["Company"]}) - ${exec_row["Total Compensation"]:,.0f}'
            )
        ...
        for i, (_, company_row) in enumerate(company_totals.iterrows(), 1):
            print(
                f'   {i:2d}. {company_row["Company"]} (Rank {company_row["Fortune Rank"]}) - ${company_row["Total Executive Pay"]:,.0f}'
            )
        ...
        print(
            f'     • Avg Executive Pay: ${fortune_1_50["Total Compensation"].mean():,.0f}'
        )
        print(
            f'     • Total Compensation: ${fortune_1_50["Total Compensation"].sum()/1_000_000_000:.1f}B'
        )
        ...
        print(
            f'     • Avg Executive Pay: ${fortune_51_100["Total Compensation"].mean():,.0f}'
        )
        print(
            f'     • Total Compensation: ${fortune_51_100["Total Compensation"].sum()/1_000_000_000:.1f}B'
        )
