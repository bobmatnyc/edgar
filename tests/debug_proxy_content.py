...
def debug_proxy_content():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            summary_comp_text = soup.find_all(
                text=re.compile(r"summary compensation table", re.IGNORECASE)
            )
            print(
                f"Found {len(summary_comp_text)} instances of 'summary compensation table'"
            )
                    ...
                    if parent:
                        ...
                        if next_table:
                            ...
                            for j, row in enumerate(rows):
                                ...
                                row_text = " | ".join(
                                    [cell.get_text().strip()[:50] for cell in cells[:6]]
                                )
            ...
            for exec_name in known_execs:
                matches = soup.find_all(
                    text=re.compile(re.escape(exec_name), re.IGNORECASE)
                )
                ...
                for match in matches[:2]:  # Show first 2
                    ...
                    if parent and parent.name == "table":
                        ...
                        if row:
                            cells = row.find_all(["td", "th"])
                            row_text = " | ".join(
                                [cell.get_text().strip()[:30] for cell in cells]
                            )
                            ...
                            if any(
                                ...
                                print(
                                    f"    *** This looks like a compensation row! ***"
                                )
                                ...
                                if table:
                                    ...
                                    # Show header row
                                    if all_rows:
                                        header_cells = all_rows[0].find_all(
                                            ["td", "th"]
                                        )
                                        header_text = " | ".join(
                                            [
                                                cell.get_text().strip()[:20]
