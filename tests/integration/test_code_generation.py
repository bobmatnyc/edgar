...
def weather_project_config():
    ...
    return ProjectConfig(
        ...
        examples=[
            ExampleConfig(input=ex["input"], output=ex["output"])
            for ex in [
                {
                    "input": {
                        "name": "London",
...
async def test_generate_weather_extractor(
    code_generator, weather_examples, weather_project_config
):
    """
    ...
    assert (
        "@pytest.mark.asyncio" in code.tests_code
        or "async def test_" in code.tests_code
    )
...
async def test_files_written_to_disk(
    code_generator, weather_examples, weather_project_config, tmp_path
):
...
async def test_minimal_examples_still_generates(code_generator):
    ...
    minimal_examples = [{"input": {"value": 10}, "output": {"result": 20}}]
...
async def test_iterative_refinement_on_validation_failure(
    code_generator, weather_examples, weather_project_config, monkeypatch
):
    """
    ...
    ), f"Expected 2 validation attempts, got {len(validation_attempts)}"
...
async def test_max_retries_exceeded(
    ...
    assert (
        len(validation_attempts) == max_retries
