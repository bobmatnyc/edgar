...
def verify_skip_behavior():
    ...
    print("-" * 80)
    output1, code1 = run_test(
        "tests/unit/services/test_code_generator_progress.py::TestProgressCallbacks::test_progress_callback_invoked_for_all_steps"
    )

    if "SKIPPED" in output1 or "skipped" in output1.lower():
        print("✓ PASS: Test skipped gracefully")
        print("  Found 'SKIPPED' in output")
    else:
    ...
    print("-" * 80)
    output2, code2 = run_test(
        "tests/integration/test_weather_api_e2e.py::TestWeatherAPILifecycle::test_weather_api_complete_lifecycle"
    )
    ...
    print("-" * 80)
    output3, code3 = run_test(
        "tests/unit/services/test_code_generator_progress.py::TestGenerationProgress"
    )
