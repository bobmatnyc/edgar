...
async def run_complete_fortune_100_analysis():
    ...
    print(
        f"✅ Loaded real Fortune 1-8 data: {len(real_f18_data['companies'])} companies"
    )
    ...
    print(
        f"✅ Loaded existing Fortune 9-100 data: {len(existing_f100_data['companies'])} companies"
    )
    ...
    for company_data in real_f18_data["companies"]:
        print(
            f"  🏢 [{company_data['rank']:2d}] {company_data['name']} - Real SEC data"
        )
...
def print_complete_summary(results: Dict, raw_file: str, excel_file: str):
    ...
    for company in fortune_1_8_companies:
        ...
        ceo_pay = next(
            (
                ...
                or "chief executive" in e.get("title", "").lower()
            ),
            0,
        )
    ...
    print(
        f"   ✅ ${total_compensation/1_000_000_000:.1f}B executive compensation validated"
    )
