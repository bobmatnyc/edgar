...
class EnhancedEdgarAnalyzer:
    ...
    async def analyze_companies(self, companies: List[Dict], limit: int = 10) -> Dict:
        ...
        for i, company in enumerate(companies[:limit]):
            ...
            try:
                ...
                print(f"  🔍 Validating {len(executives)} executives with web search...")
                validation_result = await self.validation_service.validate_executive_data(
    ...
    async def _get_proxy_content(self, cik: str) -> Optional[str]:
        ...
        try:
            ...
            result = await self.data_extraction_service.extract_executive_compensation(cik)
    ...
    async def _llm_validate_data(self, company_name: str, executives: List[Dict]) -> Optional[Dict]:
...
async def main():
    ...
    try:
        ...
        print(f"📈 Success Rate: {results['successful_extractions']/results['total_companies']*100:.1f}%")
        ...
        if results['successful_extractions'] > 0:
            print(f"🔍 Average issues per company: {total_issues/results['successful_extractions']:.1f}")
            print(f"🎯 High confidence results: {high_confidence}/{results['successful_extractions']}")
