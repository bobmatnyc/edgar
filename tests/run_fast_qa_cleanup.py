...
async def run_fast_qa_cleanup():
    ...
    qa_controller = ComprehensiveQAController(
        llm_service=None, web_search_enabled=False
    )
...
async def create_target_document_format(cleaned_companies: List[Dict], timestamp: str):
                        ...
                        qa_summary_data.append(
                            {
                                ...
                                "Top Issue 1": (
                                    company.get("issues", [""])[0]
                                    if company.get("issues")
                                    else ""
                                ),
                                "Top Issue 2": (
                                    company.get("issues", ["", ""])[1]
                                    if len(company.get("issues", [])) > 1
                                    else ""
                                ),
                                "Top Issue 3": (
                                    company.get("issues", ["", "", ""])[2]
                                    if len(company.get("issues", [])) > 2
                                    else ""
                                ),
    ...
    print(
        f"   📋 Sheets: Executive Pay Breakdown, List of Executives, Key Findings, QA Summary"
    )
...
def print_fast_qa_summary(qa_results: Dict, qa_file: str, cleaned_file: str):
    ...
    print(
        f"   High Quality: {summary['high_quality']} ({summary['high_quality']/total*100:.1f}%)"
    )
    print(
        f"   Medium Quality: {summary['medium_quality']} ({summary['medium_quality']/total*100:.1f}%)"
    )
    print(
        f"   Low Quality: {summary['low_quality']} ({summary['low_quality']/total*100:.1f}%)"
    )
    ...
    high_quality_companies = [
        c for c in qa_results["companies"] if c["quality_level"] == "HIGH"
    ]
    medium_quality_companies = [
        c for c in qa_results["companies"] if c["quality_level"] == "MEDIUM"
    ]
    ...
    if high_quality_companies:
        ...
        for company in high_quality_companies[:10]:  # Show top 10
            print(
                f"      • {company['name']} (Rank {company['rank']}) - {company['confidence_score']:.2f}"
            )
    ...
    print(
        f"   Target Excel Document: tests/results/fortune_50_executive_compensation_*.xlsx"
    )
