...
async def run_fortune_100_analysis():
    ...
    for i, company in enumerate(FORTUNE_100_COMPANIES):
        ...
        try:
            ...
            extraction_result = (
                await data_extraction_service.extract_executive_compensation(cik)
            )
            ...
            print(
                f"  🔍 Running QA validation on {len(extraction_result['executives'])} executives..."
            )
...
async def generate_enhanced_excel_report(results: Dict, timestamp: str) -> str:
    ...
    for company in results["cleaned_data"]:
        ...
        company_confidence = (
            qa_result.get("confidence_score", 0.0) if qa_result else 0.0
        )
        company_quality = (
            qa_result.get("quality_level", "UNKNOWN") if qa_result else "UNKNOWN"
        )
            ...
            qa_summary_data.append(
                {
                    ...
                    "Top Issue 1": (
                        qa_result.get("issues", [""])[0]
                        if qa_result.get("issues")
                        else ""
                    ),
                    "Top Issue 2": (
                        qa_result.get("issues", ["", ""])[1]
                        if len(qa_result.get("issues", [])) > 1
                        else ""
                    ),
                    "Top Issue 3": (
                        qa_result.get("issues", ["", "", ""])[2]
                        if len(qa_result.get("issues", [])) > 2
                        else ""
                    ),
                    "Data Status": (
                        "INCLUDED"
