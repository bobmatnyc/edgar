...
async def run_top_100_enhanced():
    ...
    data_extraction = DataExtractionService(
        edgar_api, company_service, cache_service, llm_service
    )
    ...
    for i, company in enumerate(top_100, 1):
        ...
        try:
            ...
            compensations = await data_extraction.extract_executive_compensation(
                cik, year
            )
            ...
            if compensations:
                ...
                for comp in compensations:
                    exec_data = {
                        ...
                        "stock_awards": (
                            float(comp.stock_awards) if comp.stock_awards else 0
                        ),
                        "option_awards": (
                            float(comp.option_awards) if comp.option_awards else 0
                        ),
                    }
                ...
                for comp in compensations[:3]:  # Show first 3
                    print(
                        f"     • {comp.executive_name} ({comp.title}) - ${comp.total_compensation:,}"
                    )
    ...
    if successful_companies:
        ...
        for company in successful_companies[:5]:  # Show first 5
            ...
            for exec in company["executives"][:2]:  # Show top 2 executives
                print(
                    f"     • {exec['name']} ({exec['title']}) - ${exec['total_compensation']:,.0f}"
                )
