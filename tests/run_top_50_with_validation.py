...
async def analyze_with_validation():
    ...
    for i, company in enumerate(original_results['companies']):
        ...
        try:
            ...
            enhanced_results['validation_summary']['total_issues'] += len(validation_result.issues)
            enhanced_results['validation_summary']['total_suggestions'] += len(validation_result.suggestions)
            
            print(f"  ✅ Validation complete - Confidence: {confidence:.2f} ({confidence_level})")
            ...
            enhanced_results['companies'].append({
                **company,
                'validation': {
                    ...
                    'web_search_performed': bool(validation_result.web_search_results)
                },
...
async def enhanced_llm_validation(llm_service: LLMService, company_name: str, 
                                executives: List[Dict], validation_result) -> Dict:
    ...
    try:
        ...
        ], temperature=0.2, enable_web_search=True)
...
def print_validation_summary(results: Dict, output_file: str):
    ...
    print(f"   Average Issues per Company: {validation_summary['total_issues']/successful:.1f}")
    print(f"   Average Suggestions per Company: {validation_summary['total_suggestions']/successful:.1f}")
    ...
    if high_quality > 0:
        print(f"   ✅ {high_quality} companies have high-quality data suitable for analysis")
    if medium_quality > 0:
