...
class CodeStandardsEnforcer:
    ...
    def __init__(self, project_root: Path):
        ...
        # Script directories
        self.script_dirs = ["scripts", "tests"]

        # Files to exclude from checks
        self.excluded_files = {"__pycache__", ".pyc", "__init__.py", "test_", ".git"}
    ...
    def check_all_files(self) -> List[CodeViolation]:
        ...
        logger.info("Code standards check complete", violations=len(self.violations))
    ...
    def _check_file_size(self, file_path: Path, max_lines: int = 500) -> None:
        ...
        try:
            ...
            logger.warning(
                "Could not check file size", file=str(file_path), error=str(e)
            )
    ...
    def _check_documentation_requirements(self, file_path: Path) -> None:
        ...
        try:
            ...
            logger.warning(
                "Could not check documentation", file=str(file_path), error=str(e)
            )
    ...
    def _check_service_patterns(self, file_path: Path) -> None:
        ...
        try:
            ...
            logger.warning(
                "Could not check service patterns", file=str(file_path), error=str(e)
            )
    ...
    def _check_script_header(self, file_path: Path) -> None:
        ...
        try:
            ...
            logger.warning(
                "Could not check script header", file=str(file_path), error=str(e)
            )
...
def main():
    ...
    parser.add_argument(
        "--check-core", action="store_true", help="Check core code only"
    )
    parser.add_argument(
        "--check-scripts", action="store_true", help="Check scripts only"
    )
    parser.add_argument(
        "--report-format",
