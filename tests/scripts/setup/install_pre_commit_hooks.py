    ...
    def install_hooks(self, force: bool = False) -> bool:
        ...
        try:
            ...
            if not self.git_hooks_dir.exists():
                logger.error(
                    "Git repository not found", hooks_dir=str(self.git_hooks_dir)
                )
                return False
    ...
    def uninstall_hooks(self) -> bool:
        ...
        try:
            if self.pre_commit_hook.exists():
                ...
                logger.info(
                    "Pre-commit hook removed", hook_path=str(self.pre_commit_hook)
                )
...
def main():
    ...
    parser.add_argument(
        "--uninstall", action="store_true", help="Remove pre-commit hooks"
    )
    parser.add_argument(
        "--status", action="store_true", help="Check pre-commit hook status"
    )
