...
async def test_openrouter_service():
    ...
    try:
        ...
        print(
            f"   Grok config: {grok_config['max_tokens']} tokens, web search: {grok_config['supports_web_search']}"
        )
        print(
            f"   Claude config: {claude_config['max_tokens']} tokens, web search: {claude_config['supports_web_search']}"
        )
...
async def test_llm_service_integration():
    ...
    try:
        ...
        web_response = await service.web_search_request(
            query="test query for integration", max_results=1, temperature=0.1
        )
...
async def test_model_independence():
    ...
    try:
        ...
        models_to_test = ["x-ai/grok-4.1-fast:free", "anthropic/claude-3.5-sonnet"]

        for model in models_to_test:
            try:
                response = await service.chat_completion(
                    messages=[
                        {"role": "user", "content": f"Say 'Model {model} working'"}
                    ],
                    model=model,
                    temperature=0.1,
                    max_tokens=30,
