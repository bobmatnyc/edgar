...
from edgar_analyzer.services.enhanced_data_extraction_service import EnhancedDataExtractionService
    ...
    for company in FORTUNE_1_8_COMPANIES:
        ...
        try:
            ...
            if result and result.get("success"):
                ...
                total_compensation = sum(e.get("total_compensation", 0) for e in executives)
...
async def generate_test_summary(results: Dict):
    ...
    print(f"   Successful Extractions: {successful} ({successful/total_companies*100:.1f}%)")
    ...
    if successful_results:
        ...
        successful_results.sort(key=lambda x: x.get("total_compensation", 0), reverse=True)
        ...
        total_compensation = sum(r.get("total_compensation", 0) for r in successful_results)
