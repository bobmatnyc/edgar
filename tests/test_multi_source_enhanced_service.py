...
from edgar_analyzer.services.multi_source_enhanced_service import (
    MultiSourceEnhancedService,
)
            ...
            if result["success"]:
                ...
                if attempts:
                    ...
                    for method, attempt_result in attempts:
                        ...
                        print(
                            f"      {status} {method}: {attempt_result.get('reason', attempt_result.get('error', 'Unknown'))}"
                        )
...
async def analyze_multi_source_results(results: List[Dict]):
    ...
    if multi_source_success_rate > xbrl_success_rate:
        improvement = (
            (multi_source_success_rate - xbrl_success_rate) / xbrl_success_rate * 100
        )
        print(f"   🚀 **ADDITIONAL IMPROVEMENT: +{improvement:.1f}% over XBRL alone**")

    total_improvement = (
        (multi_source_success_rate - baseline_success_rate)
        / baseline_success_rate
        * 100
    )
    print(
        f"   🎯 **TOTAL IMPROVEMENT: +{total_improvement:.1f}% over original system**"
    )
    ...
    if successful_results:
        ...
        for source, count in source_counts.items():
            print(
                f"      • {source}: {count} companies ({count/len(successful_results)*100:.1f}%)"
            )

        print(f"   🔧 **EXTRACTION METHODS:**")
        for method, count in method_counts.items():
            print(
                f"      • {method}: {count} companies ({count/len(successful_results)*100:.1f}%)"
            )

        print(f"\n💰 **AGGREGATE STATISTICS:**")
        print(f"   Total Executives Extracted: {total_executives}")
        print(
            f"   Average Executives per Company: {total_executives/len(successful_results):.1f}"
        )
        print(
            f"   Total Executive Compensation: ${total_compensation/1_000_000_000:.1f}B"
        )
        print(
            f"   Average Company Executive Pay: ${total_compensation/len(successful_results)/1_000_000:.1f}M"
        )
    ...
    if failed_results:
        ...
        for method, stats in all_attempts.items():
            ...
            print(
                f"      • {method}: {stats['failed']}/{stats['total']} failed ({failure_rate:.1f}%)"
            )
    ...
    return {
        ...
        "improvement_over_xbrl": (
            improvement if multi_source_success_rate > xbrl_success_rate else 0
        ),
