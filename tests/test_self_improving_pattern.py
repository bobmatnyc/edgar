...
from edgar_analyzer.controllers.self_improving_extraction_controller import (
    SelfImprovingExtractionController,
)
...
async def test_self_improving_pattern():
    ...
    try:
        ...
        for i, iteration in enumerate(improvement_process.get("iterations", []), 1):
            print(f"\n   Iteration {i}:")
            print(
                f"     • Test Success: {iteration.get('test_results', {}).get('success', False)}"
            )

            evaluation = iteration.get("evaluation", {})
            print(f"     • Quality Score: {evaluation.get('quality_score', 'N/A')}")
            print(
                f"     • Needs Improvement: {evaluation.get('needs_improvement', 'N/A')}"
            )
            ...
            if iteration.get("code_changed", False):
                ...
                print(
                    f"     • Files Modified: {len(changes.get('files_modified', []))}"
                )
            else:
