...
def test_submissions_api():
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print(
                f'Successfully accessed submissions for: {data.get("name", "Unknown")}'
            )
            ...
            if recent_filings:
                ...
                for i, form in enumerate(forms):
                    if form == "DEF 14A":
                        ...
                        print(
                            f"    Date: {filing_dates[i] if i < len(filing_dates) else 'N/A'}"
                        )
                        print(
                            f"    Accession: {accession_numbers[i] if i < len(accession_numbers) else 'N/A'}"
                        )
                        print(
                            f"    Primary Doc: {primary_documents[i] if i < len(primary_documents) else 'N/A'}"
                        )
