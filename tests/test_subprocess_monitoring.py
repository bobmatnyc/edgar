...
async def test_subprocess_monitoring():
    ...
    engine = DynamicScriptingEngine(prefer_subprocess=True, max_execution_time=10.0)
...
"""
    ...
    result2 = await engine.execute_script(
        script_code=error_script, context={}, safety_checks=True
    )
...
"""
    ...
    result3a = await engine.execute_script(
        script_code=comparison_script, context={}, safety_checks=True
    )
...
"""
    ...
    result4 = await engine.execute_script(
        script_code=long_script, context={}, safety_checks=True
    )
