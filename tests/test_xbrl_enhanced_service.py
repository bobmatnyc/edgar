...
from edgar_analyzer.services.xbrl_enhanced_extraction_service import (
    XBRLEnhancedExtractionService,
)
...
async def analyze_results(results: List[Dict]):
    ...
    print(
        f"   XBRL Enhanced Success Rate: {successful_extractions/total_tests*100:.1f}% ({successful_extractions}/{total_tests} companies)"
    )
...
async def main():
    ...
    with open(results_file, "w") as f:
        json.dump(
            {"timestamp": timestamp, "test_results": results, "analysis": analysis},
            f,
            indent=2,
        )
    ...
    if analysis["success_rate"] > 0.5:
        print(
            "✅ 1. XBRL Enhanced Service shows promise - integrate into main pipeline"
        )
