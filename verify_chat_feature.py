...
def verify_chat_feature():
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ClassDef) and node.name == "InteractiveExtractionSession":
            session_class = node
            break
    ...
    methods = {m.name: m for m in session_class.body if isinstance(m, (ast.FunctionDef, ast.AsyncFunctionDef))}
    ...
    if "OpenRouterClient()" not in init_source:
        print("⚠️  Warning: OpenRouterClient initialization not found in expected format")
    else:
    ...
    error_handling_checks = [
        ("Empty message check", "if not message" in cmd_chat_source or "not message.strip()" in cmd_chat_source),
        ...
        ("Progress spinner", "Progress" in cmd_chat_source or "progress" in cmd_chat_source),
        ("System prompt", "system" in cmd_chat_source.lower() and "prompt" in cmd_chat_source.lower()),
        ("Markdown rendering", "Markdown" in cmd_chat_source or "markdown" in cmd_chat_source),
    ]
